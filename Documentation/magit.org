...
- Key: g, magit-refresh
  ...
  If ~magit-auto-revert-mode~ calls for it, then it also reverts all
  unmodified buffers that visit files being tracked in the current
  repository.
...
- Key: G, magit-refresh-all
  ...
  ~magit-auto-revert-mode~ is disabled.
...
Git keeps *committed* changes around long enough for users to recover
changes that have accidentally been deleted.  It does not do the same
...
- Function: magit-run-git-async &rest args
  ...
  repository are reverted if ~magit-auto-revert-mode~ is enabled.
...
- Function: magit-start-process &rest args
  ...
  in the current repository are reverted if ~magit-auto-revert-mode~
  is enabled.
